...
/datum/action/cooldown/spell/stone_drop/cast(atom/cast_on)
	. = ..()
	prepare_stone()
	addtimer(CALLBACK(src, PROC_REF(chuck_stone), cast_on), 1 SECONDS)

/datum/action/cooldown/spell/stone_drop/proc/prepare_stone(atom/target)
...
/datum/action/cooldown/spell/stone_drop/proc/chuck_stone(atom/target)
	if(!target)
		return
...
/datum/action/cooldown/spell/stone_drop/proc/post_chuck_stone()
	return
